...
import CommentListItem from "../../components/admin/CommentListItem";
import Modal from "../../components/Modal";

const MyComments = ({ showModal, setShowModal }) => {
  ...
  const handleShowMore = async () => {
    ...
    try {
      const res = await fetch(
        `/api/comment/getCreatorPostComment/${currentUser._id}?startIndex=${startIndex}`
      );
      const data = await res.json();
      if (!res.ok) {
  ...
  const handleDeleteComment = async () => {
    try {
      const res = await fetch(
        `/api/comment/deleteCommentByCreator/${currentUser._id}/${commentIdToDelete}`,
        {
          method: "DELETE",
        }
      );
